...
| `preflight.sh` | `preflight` | Gates before any disk write: root, UEFI, Secure Boot off, `RYOKU_DISK` a whole disk >= 32 GiB, the repo payload present; for `alongside` also GPT, a usable EF00 ESP with >= 8 MiB free (shares the scanned one, Windows-first, when several exist), a free region >= `2 + 20 + swap` GiB, and a BitLocker-neighbor warning. | Nothing. | Fatal `die` with firmware/guidance text; dry-run narrates and returns. |
...
| `bootloader.sh` | `bootloader` (+ post-AUR finalize) | Installs and brands Limine and builds the kernel cmdline. Whole disk: writes the ESP-root `limine.conf`, drops the EFI binary + registers NVRAM, and after the AUR step repoints the menu at the tool-managed UKI tree. Alongside (any existing OS -- Windows or another Linux): kernels on the XBOOTLDR `/boot`, then shares the existing ESP -- tars it to a backup, drops `/EFI/ryoku/BOOTX64.EFI` + `limine.conf` (kernels by FAT label `fslabel(RYOKUBOOT)`, `guid()` chainload of the existing OS), never touching a foreign vendor dir; pins `EFI_REGISTER=no` so limine's own hook cannot add a second NVRAM entry, and installs `ryoku-limine-stage.hook` to refresh both stage loaders on limine upgrades. | `limine.conf` (wipe: `/mnt/boot`; alongside: the XBOOTLDR + `/EFI/ryoku` on the shared ESP), `/etc/default/limine`, `mkinitcpio.conf.d/ryoku-vmd.conf`, the EFI binaries, the NVRAM entry, the alongside ESP backup tar. | Fatal for the core install; `efibootmgr` is best-effort; wipe `finalize` is atomic file surgery. |
