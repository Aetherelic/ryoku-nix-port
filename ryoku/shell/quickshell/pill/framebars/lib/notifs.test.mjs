...
// captured Notifs.groups shape (Notifs.qml groups getter): one entry per app,
// count = notifications in that app, preview = the notification to show.
const groups = [
...
// dismissal routing: each row keeps its app key untouched so the dismiss button
// routes Notifs.dismissApp(row.app) to exactly that group and no other.
eq(rows(groups).map(r => r.app), ["Discord", "System"], "rows preserve the app dismissal key in order");

// a group with no appName still routes: Notifs coalesces it under "System".
