...
// Spaces route on the house Ryoku.Ui kit: how many workspaces the bar shows
// (COUNT), the marker each one wears (MARKER), and a live PREVIEW card that
// mirrors the bar's own WorkspaceWidget rendering. Writes root.workspaceMode +
// workspaceStyle.
Item {
    id: page
        ...
        Column {
            ...
            Entrance {
                ...
                SettingCard {
                    ...
                    // A bar-like pill wrapping the live marker row, mirroring the real
                    // WorkspaceWidget (default glow+dot / numbers badge / magic glyph).
                    // Its colours and sizes are the bar's own, i.e. data: a preview
                    // that drifts from the bar is worse than none. The wrapper gives it
                    // the same inset a row's control band would have.
                    Item {
                    width: parent.width
                        height: 44 + (page.tk ? page.tk.gap * 2 : 24)
                    ...
                    Rectangle {
                        ...
                        anchors.verticalCenter: parent.verticalCenter
                        height: 44
                    width: Math.round(previewRow.implicitWidth) + (page.tk ? page.tk.pad : 24)
